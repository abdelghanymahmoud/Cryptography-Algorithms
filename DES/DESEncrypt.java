...
public class DESEncrypt {
    ...
    public String encrypt(File f) throws IOException {
        ...
        while (in.hasNext()) {
            res += encrpty(in.nextLine());
        }
        return res;
    }

    public String encrpty(String text) {
        ...
        for (int i = 0; i < text.length(); i += 16) {
            res += encrypt64(text.substring(i, i + 16));
        }
        return res;
    }
    ...
    private String encrypt64(String text) {
        ...
        for (int r = 0; r < 16; r++) {
            ...
            Vector<Integer> f = function(R, subkey);
            for (int i = 0; i < L.size(); i++) {
                newL.add(R.get(i));
            }
            for (int i = 0; i < R.size(); i++) {
                newR.add(L.get(i) ^ f.get(i));
            }
            L = newL;
            R = newR;
        }
        for (int i = 0; i < 32; i++) {
            m.insertElementAt(R.get(i), i);
        }
        for (int i = 32; i < 64; i++) {
            m.insertElementAt(L.get(i - 32), i);
        }
        m = mutateIPinv(m);
